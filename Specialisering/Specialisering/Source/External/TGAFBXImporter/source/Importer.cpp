...
namespace TGA
{
	namespace FBX
	{
		...
		const char* Importer::GetLastError()
		{
			return locLastError.c_str();
		}
			...
			bool bMergeDuplicateVertices)
		{
			...
			{
				locLastError = "FBX File not found! (" + someFilePath + ")";
				return false;
			}
			...
			if (!fbxImporter)
			{
				locLastError =  "Failed to create the FBX Importer! Did you call InitImporter()?";
				return false;
			}
			...
			if (!fbxImporter->Initialize(someFilePath.c_str(), -1, FbxEnvironment::Get().fbxIOSettings) || !fbxImporter->IsFBX())
			{
				...
				locLastError =  "Failed to initialize the FBX Importer! Error was: " + std::string(fbxStatus.GetErrorString()) + ".";
				return false;
			}
		...
		bool Importer::LoadAnimationA(const std::string& someFilePath, Animation& outAnimation)
		{
			...
			{
				const FbxStatus& fbxStatus = fbxImporter->GetStatus();
				locLastError =  "Failed to create the FBX Importer! Error was: " + std::string(fbxStatus.GetErrorString()) + ".";
				return false;
			}
