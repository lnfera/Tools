...
ModelVertexToPixel main(VertexInputType input)
{
    ...
    output.binormal = mul(sign(input.uvRect[1] - input.uvRect[3]) * float3(0.f, 1.f, 0.f), toWorldRotation);
    output.tangent = mul(sign(input.uvRect[0] - input.uvRect[2]) * float3(-1.f, 0.f, 0.f), toWorldRotation);
	...
	output.texCoord0 = input.instanceUV.xy + (textureRect * input.instanceUV.zw);
	
	output.vertexColor0 = input.instanceColor;	
