...
bool IsTarga(const std::wstring& aPath)
{
	size_t index = aPath.find_last_of(L".");
	std::wstring substr = aPath.substr(index +1);

	if (lstrcmpW(substr.c_str(), L"tga")==0)
	{
		return true;
	}
	else
...
Texture* TextureManager::GetTexture(const wchar_t* aTexturePath, TextureSrgbMode aSrgbMode, bool aForceReload)
{
	...
	std::wstring stringId = aTexturePath;
	...
	ERROR_PRINT("%s %s", "Failed to load resource: ", stringId.c_str());

	result = new Texture();
	
	result->myPath = aTexturePath;
	result->SetShaderResourceView(myFailedResource.Get());
	SetDebugObjectName(result->GetShaderResourceView(), stringId);
	result->myIsFailedTexture = true;

	Vector2f texSize = GetTextureSize(myFailedResource.Get());
...
Texture* TextureManager::TryGetTexture(const wchar_t* aTexturePath, TextureSrgbMode aSrgbMode, bool aForceReload)
{
	if (!aTexturePath)
	{
		aTexturePath = L"";
...
void TextureManager::OnTextureChanged(std::wstring aFile)
{
	...
	for (auto& pair : myResourceViews)
	{
		if (pair.second->myPath.compare(notWide) != std::wstring::npos)
		{
			INFO_PRINT("%s%s", "Texture changed: ", notWide.c_str());
			GetTexture(notWide.c_str(), pair.second->mySrgbMode, true);
		}
	}
}
