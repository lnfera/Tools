...
void SpriteBatchScope::Draw(const Sprite2DInstanceData* aInstances, size_t aInstanceCount)
{
	...
	for (int i = 0; i < aInstanceCount; i++)
	{
		...
		Matrix2x2f scalingMatrix = Matrix2x2f::CreateFromScale(Vector2f((instance.mySize.x) * instance.mySizeMultiplier.x, (instance.mySize.y) * instance.mySizeMultiplier.y));
		Matrix2x2f rotationMatrix = Matrix2x2f::CreateFromRotation(instance.myRotation);
...
void SpriteBatchScope::Draw(const Sprite3DInstanceData* aInstances, size_t aInstanceCount)
{
	...
	for (int i = 0; i < aInstanceCount; i++)
	{
		...
		shaderInstance.myTransform = instance.myTransform * graphicsStateStack.GetTransform();
