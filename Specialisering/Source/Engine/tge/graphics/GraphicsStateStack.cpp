...
void GraphicsStateStack::UpdateGpuStates(bool fullReset)
{
	...
	if (fullReset || state.cameraDataVersion != myGpuRenderState.cameraDataVersion)
	{
		...
		dataPtrCommon->myToCamera = Matrix4x4f::GetFastInverse(state.camera.GetTransform());
		...
		dataPtrCommon->myCameraPosition = state.camera.GetTransform().GetPosition();
	...
	if (fullReset || state.lightDataVersion != myGpuRenderState.lightDataVersion)
	{
		...
		*dataPtrLights = {};
		...
		dataPtrLights->myDirectionalLightDirection = directionalLight.GetTransform().GetForward();
...
bool GraphicsStateStack::CreateDepthStencilStates()
{
	...
	D3D11_DEPTH_STENCIL_DESC writeLessOrEqual = {};
	writeLessOrEqual.DepthEnable = true;
	writeLessOrEqual.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ZERO;
	...
	myDepthStencilStates[(int)DepthStencilState::WriteLess] = nullptr;
