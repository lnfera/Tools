...
void Tga::ModelShader::Render(const TextureResource* const* someTextures, const Model::MeshData& aModelData, const Matrix4x4f& aObToWorld, const Matrix4x4f* someBones) const
{
...

	{
		...
		ID3D11ShaderResourceView* resourceViews[NUM_SHADER_SLOTS];
		int i = 0;
		while (i < NUM_SHADER_SLOTS && myCustomTextures[i] != nullptr)
		{
			if (myCustomTextures[i] != nullptr)
			{
				resourceViews[i] = myCustomTextures[i]->GetShaderResourceView();
				i++;
			}
		}
	...
	dataPtrCommon->myObToWorld = aObToWorld * graphicsStateStack.GetTransform();
...
void Tga::ModelShader::ResetTextureResource()
{
	for (int i = 0; i < NUM_SHADER_SLOTS; i++)
	{
		myCustomTextures[i] = nullptr;
	}
}

void Tga::ModelShader::SetTextureResource(TextureResource* aTexture, int aSlot)
{
	assert(aSlot >= 0 && "SHADER SLOT IS OUT OF RANGE!");
	assert(aSlot < NUM_SHADER_SLOTS && "SHADER SLOT IS OUT OF RANGE!");

	myCustomTextures[aSlot] = aTexture;
}
