...
namespace Tga
{
	...
	public:
		...
		~TextureManager(void);
		void Init();
		Texture* GetTexture(const wchar_t* aTexturePath, TextureSrgbMode aSrgbMode = TextureSrgbMode::ForceSrgbFormat, bool aForceReload = false);
		Texture* TryGetTexture(const wchar_t* aTexturePath, TextureSrgbMode aSrgbMode = TextureSrgbMode::ForceSrgbFormat, bool aForceReload = false);
	...
	private:
		...
		std::unordered_map<std::wstring, std::unique_ptr<Texture>> myResourceViews;
