...
Tga::ScriptEditor::ScriptEditor(int aUniqueID) : Widget(aUniqueID)
{
	...
	//auto& io = ImGui::GetIO();
	//mySettings.displaySize = ImGui::GetFontSize() + 1;
	//mySettings.myNodeFont = io.Fonts->AddFontFromFileTTF("../EngineAssets/Fonts/Ldfcomicsans-jj7l.ttf", 16);
}
...
void Tga::ScriptEditor::RenderTopRow(EditorContext& /*aContext*/)
{
	...
	if (!myActiveScript.empty())
	{
		// todo implement zoom function here
...
void Tga::ScriptEditor::RenderEditor(EditorContext& /*aContext*/)
{
		...
		ImNodes::BeginNode(currentNodeId.id);
		{
			...
			// TitleBar
			{
				if (isNodeHighlighted)
				{
					...
					ImNodes::PushColorStyle(ImNodesCol_TitleBar, ImColor(0, 1, 1, 1));
				}
				else
				{
					//todo add custom colors on node registry
					ImNodes::PushColorStyle(ImNodesCol_TitleBar, ImColor(1, 1, 1, 1));
				}
			...
			float sizeMod = 1.f;
			GetNode(currentNodeId.id)->CustomUi(sizeMod);
		...
		currentNodeId = script.GetNextNodeId(currentNodeId))
	{
		...
		ImVec2 newPos(gridPos.x, gridPos.y);
